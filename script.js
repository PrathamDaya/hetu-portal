...
const initCookingGame = () => {
    ...
    const timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(timer);
            showCustomMessage('⏰ Time\'s up! Try again!', () => {
                closeGame();
            });
        }
    }, 1000);
    ...
    document.querySelectorAll('.ingredient-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (currentRecipe.ingredients.includes(ingredient) && !collectedIngredients.includes(ingredient)) {
                collectedIngredients.push(ingredient);
                item.style.opacity = '0.5';
                item.style.pointerEvents = 'none';
                
                // Update collected ingredients display
                const collectedList = document.getElementById('collected-list');
                collectedList.innerHTML = collectedIngredients.map(ing => 
                    `<span class="collected-ingredient">${ing}</span>`
                ).join('');
                
                // Check if recipe is complete
                if (collectedIngredients.length === currentRecipe.ingredients.length) {
                    clearInterval(timer);
                    showCustomMessage(`🎉 Perfect! You made ${currentRecipe.name}!`, () => {
                        // Save score
                        const score = timeLeft;
                        gameScores.cooking = Math.max(gameScores.cooking || 0, score);
                        localStorage.setItem('gameScores', JSON.stringify(gameScores));
                        closeGame();
                    });
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (scriptURL.includes('YOUR_SCRIPT_ID_HERE') || scriptURL === '') {
        ...
        showCustomMessage('Please configure your Google Apps Script URL in script.js for full functionality.');
    }
    ...
    if (overlay) {
        overlay.addEventListener('click', closeMissYouPopup);
    }
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            ...
            // Close custom modals
            const customOverlay = document.getElementById('customMessageOverlay');
            ...
            // Close love letter
            const loveLetterModal = document.getElementById('loveLetterModal');
            if (loveLetterModal && loveLetterModal.style.display === 'flex') {
                closeLoveLetter();
            }
        }
    });
    ...
    if (typeof navigateToApp === 'undefined') {
        ...
        showCustomMessage('Error: Critical script functions not loaded properly.');
    } else {
        console.log('✅ All core functions are available and ready.');
    }
});
...
window.navigateToFeelingsPage = navigateToFeelingsPage;
window.submitFeelingsEntry = submitFeelingsEntry;
window.fetchAndDisplayFeelingsEntries = fetchAndDisplayFeelingsEntries;
window.navigateToDiaryPage = navigateToDiaryPage;
window.openDiaryEntry = openDiaryEntry;
window.viewDiaryEntry = viewDiaryEntry;
window.submitDiaryEntry = submitDiaryEntry;
window.fetchAndDisplayAllDiaryEntries = fetchAndDisplayAllDiaryEntries;
window.submitReply = submitReply;
window.generateDare = generateDare;
